...
在可信状态感知与公平接入的基础上，端到端业务承载还需要把受限无线资源转化为用户级 QoS 满足。第~\ref{chap:perception}~章建立了退化遥测条件下多源置信度调控的分布式局部网络状态感知，第~\ref{chap:handover}~章通过局部公平性估计实现了分布式协作接入切换。然而，稳定接入并不直接等价于服务质量（QoS）满足。终端接入卫星后，业务流仍需在受限的星上带宽与发射功率下获得持续服务，链路连接优势才能转化为用户级 QoS 收益。因此，本章从接入阶段推进至资源分配阶段，研究星上分布式管控条件下基于生成式网络态势感知的切片--MAC 分层协作优化方法。
...
本章的主要贡献如下：
\begin{itemize}
\item 建立随机业务到达与时变服务能力下的双时间尺度切片--MAC 资源分配模型，将资源高效的用户级 QoS 保障分解为慢时间尺度切片级资源预留子问题与快时间尺度 MAC 层业务流调度子问题，并以带宽--功率预算作为两层之间的跨层接口。
