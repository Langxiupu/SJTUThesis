...
本章的主要贡献总结如下：
\begin{itemize}
\item 将资源高效的用户级 QoS 保障建模为随机业务到达与时变服务能力下的双时间尺度切片—MAC 资源分配问题，并以带宽与功率预算为跨层接口，将原始耦合的混合整数非线性问题分解为预测式切片预算控制与时隙级 QoS 可行 MAC 调度两个子问题，在保持跨层协同的同时显著压缩联合决策空间。

\item 针对 QoS 可行性鸿沟在切片层的成因，提出基于世界模型的生成式切片资源预留机制，学习预算条件下的 QoS 可行性动态而非仅预测业务需求；以先验—后验隐空间动态用 MAC 层反馈校准想象的切片轨迹，无需在线多步随机规划即可支撑长程策略学习。
