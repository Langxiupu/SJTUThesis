\chapter{面向用户级 QoS 保障的接入网切片生成式资源预留与跨层优化机制}
\label{chap:resource}

\section{引言}

沿“感知、接入、资源分配与传输”的端到端业务承载链路，第~\ref{chap:perception}~章建立了退化遥测条件下可信的本地多跳状态感知，第~\ref{chap:handover}~章面向海量终端切换保障了接入连续性与长期公平性。然而，稳定接入并不直接等价于服务质量（QoS）满足。终端接入卫星后，业务流仍需在受限的星上带宽与发射功率下获得持续服务，链路连接优势才能转化为用户级 QoS 收益。因此，本章从接入阶段推进至资源分配阶段，研究星上分布式管控条件下面向资源使用效率与用户级 QoS 协同保障的切片--MAC 跨层无线资源分配方法。

在承载异构业务的 LEO 卫星接入网（LEO satellite Radio Access Network，LEO-sRAN）中，单颗卫星的下行带宽与发射功率受星上载荷约束，资源预算需要服务于用户级 QoS，而不能仅追求聚合吞吐。困难主要来自切片层预算与 MAC 层执行之间的时间尺度差异：一次切片层预算通常要经过后续多个媒体接入控制（MAC）时隙内的业务到达、信道起伏和流级竞争，才会体现为业务流是否满足速率--时延需求。聚合吞吐与用户级 QoS 之间也存在内在差异。如图~\ref{fig:ch4_pf_scheduling}~所示，即便总传输速率超过名义门限，业务流密度上升后仍会有部分应用流违背其速率--时延约束。因此，LEO-sRAN 资源分配需要刻画单个业务流在当前预算和信道条件下的 QoS 满足条件，而不是仅依据系统级吞吐量进行调度~\cite{sui-ConvexOptDLResAlloc-TCCN24}。
...
无线接入网切片（Radio Access Network Slicing，RAN Slicing）将 QoS 需求相近的业务归并为逻辑切片，并在切片层预留带宽与功率~\cite{li-HierarchicalSoftRANSlicing-WirelessComms20}，为差异化业务承载提供了清晰的资源组织方式。该分层结构也带来新的约束：切片层通常只能处理粗粒度业务统计，难以直接观察时隙级流竞争；MAC 层能够感知实时队列和信道，却受已经确定的切片预算约束。若切片层仅依据业务预测或已实现性能调整预算，MAC 层仅按队列紧迫度尽力调度，二者之间便缺少从预算配置到流级 QoS 满足的映射关系。由此形成跨时间尺度资源协同问题：切片层需要预测当前预算对未来用户级 QoS 的支撑能力，MAC 层需要将该预算落实为满足速率--时延门限的业务流调度。

现有研究尚未充分打通这一跨时间尺度映射。切片层资源配置多以业务需求预测或历史性能反馈为依据~\cite{liu-reconfigurableRanSlicing-TCCN25,chen-InfoRanSlicing-JSAC24,Phyu-MultiSlicePrivacy-TNSM23}，MAC 层调度则主要围绕聚合吞吐、队列稳定性或积压紧迫度展开~\cite{leng-SchedulingResourceSat-ChinaCom22,Villegas-OptimizingQoS-TNSM26}。这两类方法均未在资源分配前判断业务流的速率--时延门限在当前预算下是否可满足，因而难以避免有限带宽与功率被分配给当前无法满足 QoS 目标的业务流。对于业务到达和星地信道快速变化的 LEO-sRAN，仅依赖事后反馈的反应式调度难以形成稳定的切片--MAC 协同机制。

针对上述问题，本章提出面向 LEO-sRAN 的双时间尺度预测调度（Dual-Timescale Predictive Scheduling，DTPS）框架。DTPS 以带宽与功率预算作为切片层和 MAC 层之间的跨层接口，将长期 QoS 保障分解为慢时间尺度的预测式资源预留和快时间尺度的业务流调度。切片层通过反馈校正的业务--信道演化预测模型，生成候选预算下的用户级 QoS 演化轨迹，在预算决策前评估不同资源配置对长期 QoS 满足率的支撑能力；MAC 层则估计候选业务流满足速率--时延门限所需的最小资源块组（Resource Block Group，RBG）和功率开销，仅将 QoS 可满足的业务流纳入调度，并依据 QCI/5QI 优先级在固定切片预算内完成流选择与资源映射。通过这一接口，切片层负责维持长期 QoS 支撑能力，MAC 层负责在当前时隙将有限带宽与功率转化为实际 QoS 收益。

本章的主要贡献如下：
\begin{itemize}
\item 将资源高效的用户级 QoS 保障建模为随机业务到达与时变服务能力下的双时间尺度切片--MAC 资源分配问题。该建模以带宽与功率预算为跨层接口，将切片级资源预留与时隙级业务流调度相衔接，使切片层维持长期 QoS 支撑能力，MAC 层在固定预算内执行 QoS 可满足的业务流选择。

\item 针对切片层难以前瞻评估预算效果的问题，提出生成式切片资源预留机制。该机制通过反馈校正的业务--信道演化预测模型，刻画业务到达、信道服务能力和队列状态在不同预算下的演化规律，而非仅预测业务需求；同时利用 MAC 层反馈校正预测状态，使切片层能够在预算决策前评估候选资源配置对长期 QoS 满足率的影响。

\item 针对 MAC 层难以判断当前预算下 QoS 门限是否可满足的问题，提出 QCI/5QI 优先级感知的 QoS 可行流调度器。该调度器将候选业务流满足速率--时延门限所需的最小 RBG--功率开销作为准入依据，并在切片预算内优先选择业务优先级高且 QoS 可满足的业务流。

\item Starlink 规模星座仿真表明，DTPS 在过载业务和突发业务增强条件下均能保持较高的用户级 QoS 满足率，并在资源消耗与平均时延之间取得稳定折中；消融与固定预算调度对比验证了切片层预测式资源预留和 MAC 层 QoS 可行性测试的作用。
\end{itemize}
...
如图~\ref{fig:ch4_system_model}~所示，考虑一个 LEO 卫星接入网为某一地理区域提供下行接入服务，该区域同时处于多颗 LEO 卫星覆盖之下。每颗卫星采用多波束天线，以直连入网（Direct-to-Cell）方式服务其覆盖范围内的地面终端。地面终端产生异构业务，既包含时延敏感型业务，也包含带宽密集型业务。为提供差异化 QoS 保障，这些业务按主导 QoS 需求归并为两类逻辑切片：无线资源先在切片层预留，再由 MAC 层调度至各业务流。

考虑到星地回程链路可能具有较长时延并存在间歇性可用问题~\cite{zhou-FeederLinkHandover-Sensors23}，下行无线资源管理在每颗卫星本地执行，而不依赖核心网实时集中控制；切片上下文仅在回程链路条件允许时机会式上报核心网。可见卫星与波束间的同信道干扰假定已通过波束赋形传输调度与跳波束机制抑制~\cite{wang-CollinearInterference-TCOM25}，故不计入本章建模。在该架构下，LEO-sRAN 资源分配由两层决策构成：慢时间尺度的切片资源预留确定各切片预算，快时间尺度的 MAC 调度在该预算内分配用户级资源。
...
考虑卫星集合 $\mathcal S=\{1,\dots,S\}$ 在某一地理覆盖区域内为地面终端集合 $\mathcal U=\{1,\dots,U\}$ 提供下行接入。由于 LEO 卫星相对地面高速运动，而地面终端在所考虑时间范围内移动缓慢，终端位置可近似视为静止。卫星以离散时隙运行，时隙为最小资源分配单元。记 $\mathcal T=\{1,\dots,H\}$ 为长度为 $H$ 个时隙的调度区间，在该观测区间内卫星集合 $\mathcal S$ 与目标区域之间的可见关系保持不变，因而可见卫星集合固定。
...
同一终端可同时承载多个 QoS 需求不同的逻辑应用。因此，本章以应用流作为资源分配与 QoS 保障的基本服务单元：地面终端产生的每个逻辑应用视为一条应用流，以 $i$ 索引，全体应用流集合记为 $\mathcal I$；终端作为应用流的物理载体，记 $u(i)\in\mathcal U$ 为承载流 $i$ 的终端。应用流经其载体终端的星地链路传输，其服务卫星由终端关联决定，卫星 $s$ 在间隔 $k$ 内服务的应用流集合为 $\mathcal I_s(k)\triangleq\{i\in\mathcal I\mid a_{u(i),s}(k)=1\}$。下文“业务流”均指此类应用流。
...
二值 QoS 指示只能记录需求是否被违背，难以描述当前服务状态与门限之间的裕度。这里引入连续 QoS 度量有两方面作用：切片层世界模型需要预测不同预算下 QoS 的渐进退化与恢复，MAC 调度器也需要在估计 RBG--功率开销前搜索满足速率--时延联合门限的最小服务速率。为此采用 sigmoid 效用函数，在以所需速率与时延为中心逼近阶跃门限的同时，提供有界且光滑的 QoS 评分~\cite{sun-ResourceSlicing-systemJournal20}。记业务流 $i$ 在时隙 $t$ 的实际速率与时延分别为 $r_i(t)$ 与 $d_i(t)$，速率效用定义为
...
为获得可在线执行的资源分配机制，本节依据 LEO-sRAN 的物理控制层级对 $\mathbf{P1}$ 进行分解。该分解不追求 $\mathbf{P1}$ 的集中式全局最优解，而是以切片级带宽与功率预算保留切片--MAC 耦合关系，同时避免直接联合优化关联、RBG 分配与功率分配。分解遵循三条原则。

其一，以单星本地协调替代全局多星优化。每颗卫星依据本地业务观测与邻星上下文执行资源管理；用户关联与业务迁移通过可见卫星间的本地协调调整，以缓解空间需求不均。

其二，将长期随机优化转化为离线动态学习与在线资源决策。预测模型离线学习资源预算在随机业务与信道变化下对未来 QoS 满足率的影响；在线执行时，调度器依据当前切片观测和已学习的服务动态推断资源预算，而不在每个间隔求解长视野随机规划。所提世界模型利用 MAC 反馈校正离线学习与在线服务状态之间的偏差，即在后续预算决策前用最新 MAC 层服务统计更新切片服务状态。

其三，分解遵循可行域参数化原则。上层不直接确定时隙级用户调度，而是通过切片级带宽与功率预算限定下层 MAC 问题的可行集。对任意满足卫星级资源约束的非负预算，MAC 问题均具有良定义的可行域，并包含无任何业务流可满足其 QoS 需求时的空调度动作；MAC 调度器仅在该预算诱导的可行域内选择用户并分配资源。

这一分解降低了两层决策规模：切片层基于聚合的切片级描述符与业务统计运行，不处理流级 RBG 分配；MAC 层仅在切片层确定的预算内求解时隙级选择问题。由此，预算接口将长期预测式预留与快时间尺度资源执行分离，同时保留用户级 QoS 保障所需的跨层协同。
...
本节给出双时间尺度预测调度（DTPS）框架。DTPS 由两个协同模块构成：慢时间尺度上基于世界模型的生成式切片资源预留模块（求解 $\mathbf{P2}$），以及快时间尺度上基于 QCI 优先级背包的 MAC 调度模块（求解 $\mathbf{P3}$）。切片层预测切片级带宽与功率预算，MAC 层在时隙级将该预算映射为 QoS 可满足的业务流调度。两模块以带宽与功率预算为跨层接口，以 MAC 层服务统计作为反馈。本节先给出总体框架，再阐述两个模块的内部机制，并分析整体工作流与复杂度。

\subsection{总体框架设计}
\label{subsec:ch4_overview}

DTPS 将 $\mathbf{P1}$ 的求解沿物理控制层级分置于两个时间尺度不同、功能互补的模块，二者通过切片级预算耦合。

慢时间尺度的\textbf{生成式切片资源预留模块}求解 $\mathbf{P2}$。该模块以聚合的切片级业务统计与有限邻星上下文为输入，输出各切片的带宽预算 $\{W_{s,c}(k)\}$、功率预算 $\{P_{s,c}(k)\}$ 以及本地关联调整 $\mathbf m_s(k)$；它不涉及时隙级 RBG 指派或流级功率分配，而是界定下层 MAC 执行的可行域。由于一次切片预算对用户级 QoS 的影响需要经过后续多个 MAC 时隙内的业务竞争、到达波动和信道变化才能显现，该模块通过世界模型学习预算条件下的 QoS 动态，在预算决策前评估其长期效果。

快时间尺度的\textbf{QCI 优先级背包 MAC 调度模块}求解 $\mathbf{P3}$。给定切片层确定的带宽与功率预算，该模块在预算诱导的可行域内决定当前时隙服务哪些业务流，并保证被选中流以所消耗资源满足其 QoS 需求；它不再压缩预留预算，而是在固定预算下服务更多高优先级业务流。

两模块的功能边界由资源预算接口确定：切片层使用粗粒度统计，面向长期资源预留，不触及时隙级调度；MAC 层在固定预算可行域内选流，面向实时 QoS 满足，不重新规划切片预算。跨层反馈闭环与两模块的时序关系见第~\ref{subsec:ch4_workflow}~节（图~\ref{fig:ch4_dtps_workflow}）。

\subsection{基于世界模型的生成式切片资源预留}
\label{subsec:ch4_worldmodel}

在一个切片间隔内，业务到达、信道状态与资源竞争跨多个 MAC 时隙变化，而切片层只能在 MAC 执行后观测聚合业务统计。这类稀疏观测使切片层难以判断当前带宽--功率预算将如何影响未来用户级 QoS。为此，本节设计基于世界模型的生成式切片资源预留模块。该模块在内部状态空间中学习预算条件下的服务状态演化，并生成未来切片服务预测，从而在预算决策前评估 $\mathbf{P2}$ 所需的长期 QoS 可行性。

该模块沿用 Dreamer 类世界模型的时序状态建模思想~\cite{Hafner-WorldModels-arXiv24}，并针对 LEO-sRAN 资源分配作三点适配。第一，观测由切片级服务能力、业务类型需求、资源状态与邻星上下文构成，而不是视觉或通用控制状态。第二，预测分支直接输出通信性能量，包括预留代价、QoS 违背率与切片级 QoS 满足。第三，学习得到的服务状态预测模型与基于近端策略优化（Proximal Policy Optimization，PPO）的切片策略耦合，用于确定带宽--功率预算与本地关联调整；流级 QoS 可满足性仍由 MAC 层独立判定。经此适配，切片层学习的是预算条件下的 QoS 动态，而非仅预测未来业务需求。据此将慢时间尺度问题 $\mathbf{P2}$ 建模为部分可观马尔可夫决策过程（Partially Observable Markov Decision Process，POMDP），其观测、动作与奖励定义如下。
...
\textbf{切片服务状态跟踪与 MAC 反馈校正.} 该模块不学习通用环境动态，而是在切片间隔尺度上跟踪资源预算、服务能力与 QoS 违背之间的演化关系。为此，引入两类内部状态变量：历史服务状态记忆 $\boldsymbol h_k$ 用于汇总过去切片观测、资源预算和 MAC 执行结果；不确定服务状态变量 $\boldsymbol z_k$ 用于刻画业务到达、信道服务能力和 QoS 压力中的随机波动。二者共同构成切片层预算决策所需的内部状态表征 $(\boldsymbol h_k,\boldsymbol z_k)$。历史服务状态记忆由长短期记忆（Long Short-Term Memory，LSTM）网络 $f_\theta(\cdot)$ 更新~\cite{Gong-SatTrafficPrediction-ICCC24}：
...
在此基础上，将上述服务能力预测模型与基于近端策略优化（Proximal Policy Optimization，PPO）的切片策略结合~\cite{Li-EnergyBalancingOffloading-TGCN26}。这里 PPO 仅作为切片层预算策略的训练方法，其输出仍是通信意义明确的带宽预算、功率预算和本地关联调整；流级 QoS 可行性由第~\ref{subsec:ch4_mac}~节的 MAC 调度器独立保证。训练过程中，世界模型与切片策略交替更新。世界模型训练阶段利用真实执行轨迹进行状态校正、观测重构和性能预测，并通过预测损失与状态预测一致性损失更新记忆模块、状态预测分布、反馈校正分布和性能预测分支。策略训练阶段固定世界模型参数，并将其作为模型内预测器：从近期采集轨迹中的反馈校正状态出发，在切片策略给出的预算动作下递推未来服务状态，获得预留代价、QoS 违背率与长期奖励，随后更新切片策略网络和价值函数网络。采用近期轨迹批中的反馈校正状态初始化模型内前向预测，而非仅从最新轨迹的终止状态出发，可以提高切片状态覆盖范围并稳定长视野预算策略训练。算法~\ref{algo:ch4_slicing} 给出该交替优化流程。
...
本章在分布式管控范式下，研究大规模 LEO 卫星接入网（LEO-sRAN）中面向资源使用效率与用户级 QoS 协同保障的接入网切片与跨层无线资源优化。单颗卫星需要在受限星上带宽与发射功率下，为已接入用户的异构业务分配资源；切片层粗粒度预算只有经过后续多个 MAC 时隙中的业务到达、信道变化与流级竞争，才会体现为用户级 QoS 满足。由于聚合吞吐并不等价于用户级 QoS 满足，仅以吞吐或队列紧迫度为依据的资源分配难以将有限带宽与功率转化为有效 QoS 收益。为此，本章提出双时间尺度预测调度（DTPS）框架，以带宽与功率预算为跨层接口，将单星本地切片--MAC 资源分配问题分解为切片层的 QoS 可行域保障与 MAC 层的实时调度两个子问题。其中，慢时间尺度的生成式切片资源预留模块通过反馈校正的业务--信道演化预测模型，评估候选预算对长期 QoS 满足率的影响；快时间尺度的 QCI 优先级背包调度模块先估计各候选流满足速率--时延联合门限所需的最小 RBG--功率开销，再在固定切片预算下选取优先级加权的可行业务流集合。

Starlink 规模星座仿真结果表明，DTPS 在过载业务和突发业务增强条件下均能保持较高的用户级 QoS 满足率，并避免将有限带宽与功率消耗在难以满足 QoS 门限的业务流上。消融结果表明，反馈校正的切片层预测模型、长视野预算评估和在线校正分别支撑长期 QoS 保障、资源预留稳定性和突发鲁棒性。固定切片预算下的调度对比说明，MAC 层在资源分配前进行 QoS 可行性测试，比单纯依据队列紧迫度或业务优先级调度更能提升 QoS 满足率。

上述结果表明，在资源受限的 LEO-sRAN 中，资源分配的重点不是单纯提升聚合吞吐，而是将有限带宽与功率优先映射为可满足的用户级 QoS 收益。本章面向资源分配环节建立的跨层资源优化机制，与第~\ref{chap:perception}~章的状态感知、第~\ref{chap:handover}~章的接入切换共同覆盖了端到端业务承载链路的前三个环节；第~\ref{chap:semantic}~章将推进至传输环节，在受限星上载荷与链路条件下研究端到端传输性能与硬件开销之间的协同问题。
