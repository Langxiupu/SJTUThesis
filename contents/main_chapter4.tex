...
沿“感知—接入—调度—传输”的端到端数据通路，第~\ref{chap:perception}~章构建了退化遥测条件下可信的本地多跳状态感知，第~\ref{chap:handover}~章保障了海量终端接入的连续性与公平性。然而，稳定接入只是端到端服务质量（QoS）保障的起点：终端接入后，卫星仍须在受限的星上带宽与发射功率下为其异构业务分配无线资源，才能将链路连接质量转化为用户级 QoS 满足。由此，本章从“接入”推进至“调度”，研究纯分布式管控范式下面向用户级 QoS 保障的接入网切片与跨层无线资源优化。
