...
无线接入网切片（Radio Access Network Slicing，RAN Slicing）将 QoS 需求相近的业务归并为逻辑切片并在切片层预留带宽与功率~\cite{li-HierarchicalSoftRANSlicing-WirelessComms20}，为此提供了一种可处理的问题结构，但切片—MAC 分层也在粗粒度预算与用户级 QoS 满足之间引入了 QoS 可行性鸿沟：切片层多预测业务需求或从已实现回报学习策略，未刻画当前预算经 MAC 竞争与信道变化后如何影响未来用户级 QoS 可行性；MAC 层则以队列紧迫度尽力而为地调度，不判定当前预算下 QoS 目标是否可达。这构成一个跨时间尺度的可行性问题：切片层须预测由当前预算到未来 QoS 可行性的映射，MAC 层须将预算转化为 QoS 可行的用户级调度。

然而，现有研究尚未将这一跨时间尺度的可行性映射贯通：切片层资源配置仍以业务需求预测或已实现回报为依据~\cite{liu-reconfigurableRanSlicing-TCCN25,chen-InfoRanSlicing-JSAC24,Phyu-MultiSlicePrivacy-TNSM23}，MAC 层调度仍以聚合吞吐、队列稳定性或积压紧迫度为准则~\cite{leng-SchedulingResourceSat-ChinaCom22,Villegas-OptimizingQoS-TNSM26}，二者均未判定 QoS 门限在当前预算下是否可达。这种相互割裂的反应式调度无法形成切片—MAC 跨层协同，难以适应 LEO 卫星网络的高动态性。因此，仍缺乏一种预测当前预算所诱导的用户级 QoS 可行性、并将其转化为可行 MAC 调度的切片—MAC 协同机制。

针对上述问题，本章提出面向 LEO-sRAN 的双时间尺度预测调度（DTPS）框架，以带宽与功率预算为跨层接口、以 QoS 可行性为切片层与 MAC 层协同的核心准则，将面向长期 QoS 保障的高维跨层优化分解为切片层的 QoS 可行域保障与 MAC 层的实时调度：切片层预测当前预算能否支撑未来用户级 QoS 并以预算界定 MAC 层调度的可行域，MAC 层在该可行域内仅调度 QoS 可达的业务流，使有限带宽与功率优先转化为有效 QoS 满足而非单纯聚合吞吐。切片层借鉴世界模型思想~\cite{Hafner-WorldModels-arXiv24}设计生成式切片资源预留机制，学习预算条件下的 QoS 可行性动态：先验自记忆生成想象的未来轨迹，后验以真实 MAC 层反馈校正隐空间表征，从而无需在线多步随机规划即可评估长程预算效应；MAC 层设计 QoS 类标识（QCI）优先级背包调度器，估计各候选流满足其 QoS 门限所需的最小资源块组（Resource Block Group，RBG）—功率对，仅纳入 QoS 可行流，并以 QCI/5QI 导出优先级为物品价值、所需 RBG 数与功率为权重，将 MAC 调度转化为固定预算下的二维背包选择。

本章的主要贡献总结如下：
\begin{itemize}
\item 将资源高效的用户级 QoS 保障建模为随机业务到达与时变服务能力下的双时间尺度切片—MAC 资源分配问题，并以带宽与功率预算为跨层接口，将原始耦合的高维混合整数非线性问题分解为面向 QoS 可行域保障的预测式切片预算控制与可行域内实时执行的时隙级 QoS 可行 MAC 调度两个子问题，在保持跨层协同的同时显著压缩联合决策空间。

\item 针对 QoS 可行性鸿沟在切片层的成因，提出基于世界模型的生成式切片资源预留机制，增强对网络环境演化动态的表达能力，学习预算条件下的 QoS 可行性动态而非仅预测业务需求；以先验—后验隐空间动态用 MAC 层反馈校准想象的切片轨迹，无需在线多步随机规划即可支撑长程策略学习。

\item 针对该鸿沟在 MAC 层的成因，提出 QCI 优先级背包调度器，将 MAC 执行由吞吐或紧迫度驱动转为固定预算下 QoS 可行性感知的选择：依据满足 QoS 门限所需的最小 RBG—功率开销将候选流转化为可行物品，并在切片预算下选取优先级加权的可行子集。

\item 在 Starlink 规模星座上的仿真表明，过载业务下 DTPS 的 QoS 满足率达 $80.8\%$，显著高于代表性基线的 $33.0\%$--$56.3\%$，并将加权资源消耗与平均时延分别保持在 $0.839$ 与 $78.3$~ms；突发不确定性增强时其 QoS 满足率始终维持在 $97.6\%$ 以上。
\end{itemize}
...
\end{equation}
其中 $N_0$ 为噪声功率谱密度，$B_i(t)$ 由式~\eqref{eq:ch4_userbw} 给出。由香农公式，业务流 $i$ 的可达下行速率为
...
慢时间尺度的\textbf{生成式切片资源预留模块}求解 $\mathbf{P2}$。它仅以聚合的切片级业务统计与有限邻星上下文为输入，输出各切片的带宽预算 $\{W_{s,c}(k)\}$、功率预算 $\{P_{s,c}(k)\}$ 以及本地关联调整 $\mathbf m_s(k)$；该模块不涉及时隙级 RBG 指派或流级功率分配，其作用是界定下层 MAC 执行的可行域。由于一次切片预算的 QoS 后果须在后续多个 MAC 时隙的竞争、业务到达与信道变化之后才能显现，该模块以世界模型学习预算条件下的 QoS 可行性动态，从而在预算决策前评估其长期效应。
...
生成式切片资源预留在切片层面提升了异构 QoS 保障的长期可行性。快时间尺度问题则是切片级带宽与功率预算下针对各业务流的 MAC 调度：在给定预算内，MAC 调度器须确定当前时隙服务哪些业务流，并确保被选中流以所消耗资源满足其 QoS 需求。以李雅普诺夫方法为代表的队列型调度器经积压或亏损压力间接调节 QoS，但高队列权重并不意味着用户的 QoS 需求在当前信道与切片预算下可达。服务此类用户可能耗费稀缺的 RBG 或功率，却仍无法满足其 QoS 门限。因此，本节不再最大化队列加权吞吐或额外求解连续功率分配问题，而是将 MAC 层调度形式化为 QoS 可行性感知的资源选择问题。
...
本章在纯分布式管控范式下，研究大规模 LEO 卫星接入网（LEO-sRAN）中面向资源使用效率与用户级 QoS 协同保障的接入网切片与跨层无线资源优化。在该场景中，单颗卫星须在受限星上带宽与发射功率下，为已接入用户的异构业务分配资源，而一次资源决策的 QoS 后果须经后续多个 MAC 时隙的竞争、业务到达与信道变化之后才能显现，由此在切片级粗粒度预算与用户级 QoS 之间形成 QoS 可行性鸿沟；聚合吞吐并不等价于用户级 QoS 满足，使仅以吞吐或队列紧迫度为依据的分配难以将有限资源转化为可靠的 QoS 保障，而反应式的切片资源预留与 MAC 调度又相互割裂，难以形成跨层协同以适应 LEO 卫星网络的动态性。为此，本章提出双时间尺度预测调度（DTPS）框架，以带宽与功率预算为跨层接口，将面向长期 QoS 保障的高维多星资源分配问题分解为切片层的 QoS 可行域保障与 MAC 层的实时调度两个子问题。其中，慢时间尺度的生成式切片资源预留模块以世界模型增强对环境演化动态的表达能力，学习预算条件下的 QoS 可行性动态，经先验—后验对齐与想象前向推演在预算决策前评估其长期效应；快时间尺度的 QCI 优先级背包调度模块先估计各候选流满足其速率—时延联合门限所需的最小 RBG—功率开销，再以二维背包在固定切片预算下选取优先级加权的可行子集。该框架使慢时间尺度推断与活跃业务流数无关，并将快时间尺度调度限定在受预算界定的离散搜索空间内。

在 Starlink 规模星座上的仿真表明，DTPS 在过载业务下取得 $80.8\%$ 的 QoS 满足率，显著高于代表性基线的 $33.0\%$--$56.3\%$，同时将加权资源消耗保持在 $0.839$、平均时延保持在 $78.3$~ms；在突发不确定性增强时，其 QoS 满足率在各组实验中均维持在 $97.6\%$ 以上。消融实验显示，先验—后验对齐、基于想象的策略学习与在线后验校正分别贡献了可靠动态学习、长视野策略训练与突发鲁棒性：移除先验—后验对齐使 QoS 满足率下降 $29.6$ 个百分点，移除基于想象的策略学习下降 $7.2$ 个百分点，而在线后验校正在高突发场景下带来约 $3$ 个百分点的额外增益。固定切片预算下的对比进一步表明，QoS 可行物品构造在重载下将满足率提升 $23.1$ 个百分点，QCI 感知的物品价值设计再带来最高 $11.9$ 个百分点的提升。上述结果支持本章的核心结论：在资源受限的 LEO-sRAN 中，面向可行性的 QoS 保障较面向吞吐的分配更为适宜，而在 MAC 执行前进行可行性测试比单纯的优先级设计更为根本。本章面向资源分配环节建立的跨层资源优化机制，与第~\ref{chap:perception}~章的状态感知、第~\ref{chap:handover}~章的接入切换共同覆盖了端到端传输链路的前三个环节；第~\ref{chap:semantic}~章将进一步推进至传输环节，在受限星上载荷与链路条件下研究端到端传输性能与硬件开销之间的协同问题。
