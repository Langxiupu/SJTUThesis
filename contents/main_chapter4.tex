...
沿“感知、接入、资源分配与传输”的端到端业务承载链路，第~\ref{chap:perception}~章建立了退化遥测条件下多源置信度调控的分布式局部网络状态感知，第~\ref{chap:handover}~章通过局部公平性估计实现了分布式协作接入切换。然而，稳定接入并不直接等价于服务质量（QoS）满足。终端接入卫星后，业务流仍需在受限的星上带宽与发射功率下获得持续服务，链路连接优势才能转化为用户级 QoS 收益。因此，本章从接入阶段推进至资源分配阶段，研究星上分布式管控条件下基于生成式网络态势感知的切片--MAC 分层协作优化方法。
