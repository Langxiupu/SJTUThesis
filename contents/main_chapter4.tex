...
其二，将 $\mathbf{P1}$ 中的长期随机优化转化为离线动态学习与在线资源决策。预测模型离线学习资源预算在随机业务与信道变化下如何影响未来 QoS 可行性；在线执行时，调度器依据观测到的切片状态与已学习的动态推断当前资源预算，而非在每个间隔求解长视野随机规划。所提世界模型进一步以 MAC 反馈校正缓解离线与在线之间的失配，即由最新的 MAC 层服务统计在后续预算决策前更新切片服务状态。
...
在一个切片间隔内，业务到达、信道状态与资源竞争跨多个 MAC 时隙变化，而切片层仅在 MAC 执行之后观测到聚合的业务统计。这类稀疏观测使其难以推断当前带宽—功率预算将如何影响未来的用户级 QoS 可行性。为此，本节设计基于世界模型的生成式切片资源预留模块：它在内部状态空间中学习预算条件下的服务状态演化，并据此生成未来切片服务预测，从而在预算决策前评估 $\mathbf{P2}$ 所需的长期 QoS 可行性。

该模块沿用 Dreamer 类世界模型的时序状态建模思想~\cite{Hafner-WorldModels-arXiv24}，并针对 LEO-sRAN 资源分配作三点适配：其一，观测由切片级服务能力、业务类型需求、资源状态与邻星上下文构成，而非视觉或通用控制状态；其二，预测分支输出通信特有量，包括预留代价、QoS 违背率与切片级 QoS 满足；其三，将学习到的服务状态预测模型与基于近端策略优化（Proximal Policy Optimization，PPO）的切片策略耦合，以确定带宽—功率预算与本地关联调整，QoS 可行性则由 MAC 层经独立的资源选择机制保证。经此适配，切片层学习的是预算条件下的 QoS 可行性动态，而非仅预测未来业务需求。据此将慢时间尺度问题 $\mathbf{P2}$ 建模为部分可观马尔可夫决策过程（Partially Observable Markov Decision Process，POMDP），其观测、动作与奖励定义如下。
...
\end{equation}
其中 $q_\theta(\cdot)$ 由多层感知机（Multilayer Perceptron，MLP）参数化。式~\eqref{eq:ch4_posterior} 将已实现吞吐、时延、QoS 违背率和资源占用等 MAC 层统计重新注入切片层状态，使切片层在下一次预算决策前获得经反馈校正的服务能力估计。与之对应，历史预测分布 $p_\theta(\boldsymbol z_k\mid\boldsymbol h_k)$ 学习在缺少当前物理观测时，仅由历史服务状态记忆预测切片服务状态。通过使历史预测分布逼近反馈校正分布，模型能够在未来 MAC 反馈尚不可得时，预测不同带宽--功率预算下的切片级服务能力和 QoS 目标满足能力。

在线执行时，每个切片间隔均利用最新 MAC 层服务统计执行上述反馈校正。当业务突发、服务速率下降或用户关联调整导致实际服务状态偏离历史预测时，反馈校正可以及时修正切片层对当前可行域的估计，避免预算决策持续依赖过时的服务状态。对于未来多个切片间隔的预测，由于尚无真实 MAC 反馈，模型使用历史预测分布递推未来服务状态，从而为切片层提供预算条件下的服务能力预测。

\textbf{预测输出与训练目标.} 为支撑切片级资源预留，模型需要输出与 $\mathbf{P2}$ 直接相关的通信性能指标，而不仅重构观测状态。以 $(\boldsymbol h_k,\boldsymbol z_k)$ 为输入，模型分别预测当前观测、预留代价、QoS 违背率和切片级 QoS 满足度：
...
\end{equation}
其中 $\beta$ 权衡性能预测精度与状态预测一致性。世界模型的神经网络结构与上述各损失的反向传播路径如图~\ref{fig:ch4_wm_arch}~所示。
...
\textbf{模型内前向预测与策略优化.} 训练后的世界模型作为切片层的服务能力预测器使用。在每个切片间隔起始，模型先根据最新 MAC 反馈校正当前切片服务状态；随后，在候选带宽--功率预算和本地关联调整下，递推预测未来若干切片间隔内的预留代价、QoS 违背率和切片级 QoS 满足度。这一模型内前向预测为 $\mathbf{P2}$ 提供长期预算效果评估，而不需要在在线阶段显式求解多步随机优化问题。给定一段候选切片动作序列，内部状态递推为
...
\end{equation}
由于未来切片间隔的真实业务到达、信道服务能力和 MAC 执行结果尚不可见，前向预测阶段不使用反馈校正分布，而由历史预测分布递推未来服务状态。由预测得到的预留代价、QoS 违背率和切片级 QoS 满足度进一步构造长期奖励，用于训练切片预算策略。图~\ref{fig:ch4_wm_workflow}~在时间轴上展示了世界模型每个切片间隔的推理步骤：记忆更新、反馈状态校正、性能预测输出、历史状态预测与在线反馈校正。
...
在此基础上，将上述服务能力预测模型与基于近端策略优化（Proximal Policy Optimization，PPO）的切片策略结合~\cite{Li-EnergyBalancingOffloading-TGCN26}。这里 PPO 仅作为切片层预算策略的训练方法，其输出仍是通信意义明确的带宽预算、功率预算和本地关联调整；流级 QoS 可行性由第~\ref{subsec:ch4_mac}~节的 MAC 调度器独立保证。训练过程中，世界模型与切片策略交替更新。世界模型训练阶段，利用真实执行轨迹进行状态校正、观测重构和性能预测，并通过预测损失与状态预测一致性损失更新记忆模块、状态预测分布、反馈校正分布和性能预测分支。策略训练阶段，固定世界模型参数，并将其作为模型内预测器：从近期采集轨迹中的反馈校正状态出发，在切片策略给出的预算动作下递推未来服务状态，获得预留代价、QoS 违背率与长期奖励，随后更新切片策略网络和价值函数网络。采用近期轨迹批中的反馈校正状态初始化模型内前向预测，而非仅从最新轨迹的终止状态出发，可以提升切片状态覆盖范围并稳定长视野预算策略训练。算法~\ref{algo:ch4_slicing} 给出该交替优化流程。
...
\textbf{整体工作流.} DTPS 的端到端工作流如图~\ref{fig:ch4_dtps_workflow}~所示。在每个切片间隔起始，切片层更新记忆状态、由当前观测执行反馈状态校正，并确定切片级带宽与功率预算以及本地关联调整，这些决策参数化随后 MAC 执行的可行域。在每个 MAC 时隙内，调度器在当前大尺度信道状态下评估候选业务流的 QoS 可行资源需求，经 QCI 优先级背包选取优先级加权的可行子集，并直接执行相应的 RBG—功率分配。实现的服务统计随后反馈至切片层，触发反馈校正与后续的预测式预算，由此形成慢/快两个时间尺度上的闭环。
...
如图~\ref{fig:ch4_exp_ablation_qos} 所示，移除一致性约束变体带来最大降幅，将 QoS 满足率由 $98.0\%$ 降至 $68.4\%$，这一 $29.6$ 个百分点的下降表明状态预测一致性约束对可靠的模型内前向预测至关重要。移除模型内预测变体取得 $90.8\%$，较完整 DTPS 低 $7.2$ 个百分点，证实模型内预测策略训练对长视野预测式预算有贡献。移除反馈校正变体在中载设置下仅有边际退化，由 $98.0\%$ 降至 $97.5\%$，说明在服务动态相对平滑时，历史预测分布已能提供有效服务状态估计。图~\ref{fig:ch4_exp_ablation_burst} 进一步表明，在线反馈校正在更强突发不确定性下更为重要：在高突发概率（$p_b=0.05$）下，DTPS 维持 $97.6\%$ 的 QoS 满足率，而移除反馈校正变体降至 $94.6\%$，其标准差亦升至 $5.2$ 个百分点，相比之下 DTPS 为 $2.2$ 个百分点。总体而言，消融结果表明状态预测一致性约束、模型内前向预测与在线反馈校正分别支撑了可靠的服务状态学习、长视野策略训练与对突发服务波动的鲁棒性。
