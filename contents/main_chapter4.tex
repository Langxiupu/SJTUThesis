...
然而，现有研究尚未将这一跨时间尺度的可行性映射贯通：切片层资源配置仍以业务需求预测或已实现回报为依据~\cite{liu-reconfigurableRanSlicing-TCCN25,chen-InfoRanSlicing-JSAC24,Phyu-MultiSlicePrivacy-TNSM23}，MAC 层调度仍以聚合吞吐、队列稳定性或积压紧迫度为准则~\cite{leng-SchedulingResourceSat-ChinaCom22,Villegas-OptimizingQoS-TNSM26}，二者均未判定 QoS 门限在当前预算下是否可达。因此，仍缺乏一种预测当前预算所诱导的用户级 QoS 可行性、并将其转化为可行 MAC 调度的切片—MAC 协同机制。
