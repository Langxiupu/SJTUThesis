...
在高压缩率图像传输条件下，三轮增量传输以约 $24.5\%$ 的累计 CBR 增幅换取 $12.24$ 个百分点的 Top-1 分类准确率增益，给出了增量通信开销与任务性能之间的定量交换关系。在 LEO 卫星过顶北京密云、喀什和三亚地面站的星地图像传输仿真中，当采用相同的前向传输与反馈开销时，任务驱动反馈较随机反馈将双任务平均分类准确率提高 $4.5$--$5.4$ 个百分点，说明基于当前任务判决敏感度的分块优先级在所测试的时变星地链路配置下均取得更高的任务准确率。在当前实现的统计口径下，CIS-R 通信模型参数量约为生成式 DiffJSCC 方案的 $1/2400$。该比值反映当前通信模型的规模差异，不作为星上硬件推理开销的直接测量。
