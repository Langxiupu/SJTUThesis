...
实现级开销测试在 NVIDIA RTX A6000 GPU 上完成，测试批量大小为 $1$，数据精度为 FP32。CIS-R 的 GPU 任务处理时延从输入图像进入通信模型开始计时，至输出 R3 分类结果结束，包括基础轮与三个增量轮的编码和解码、四次任务模型前向推理、三次基于梯度的分块归因计算、分块选择及接收状态融合，不包括星地传播、前向符号传输、反馈链路传输、模型加载、主机与 GPU 数据搬移和文件读写。参数量统计通信模型，峰值显存覆盖当前实现的完整任务处理链路。开销对比中，DiffJSCC 使用 $512\times512$ 输入和 $50$ 个去噪步骤，其余方法的输入尺寸均为 $224\times224$；CIS-R 采用 $\epsilon=64$，并执行 R0 至 R3 的完整收发流程。由于输入尺寸、迭代次数和多轮交互过程不同，该对比用于刻画当前实现的模型规模与 GPU 任务处理开销，不构成同构网络结构下的严格复杂度比较。
...
表~\ref{tab:ch6_complexity_latency} 按照第~\ref{subsec:ch6_setup}~节给出的硬件环境与统计口径，比较不同方法当前实现的模型规模、GPU 任务处理时延和峰值显存占用。其中，GPU 任务处理时延属于接收端任务处理计算时延，不包括通信与传播过程，因而不等同于端到端星地业务时延。
...
CIS-R 的通信模型参数量为 $2.274$~M，峰值显存约为 $150$~MiB，均为表中最低。完整 R0--R3 GPU 任务处理时间为 $45.340$~ms。SwinJSCC 和 Codebook 只执行单次前向处理，GPU 任务处理时延低于 CIS-R；DiffJSCC 的生成式恢复包含多次迭代，GPU 任务处理时延达到 $4382.750$~ms。CIS-R 将任务模型推理和反馈计算部署在地面端，星上仅执行基础编码与语义增量生成。
...
时变星地链路实验采用第~\ref{subsec:ch6_setup}~节给出的 ImageNet-100 与 CIFAR-100 混合分类业务负载。两种数据分布分别使用相应的地面分类模型完成推理与反馈计算。任务驱动反馈和随机反馈使用相同的前向、反向负载，并对每幅图像、每个轮次采用相同的 AWGN 噪声样本。两种数据分布的汇总分类准确率列于表~\ref{tab:ch6_trace_case}。
...
\begin{table}[!htp]
    \centering
    \bicaption{时变星地链路下两种图像分类数据分布的传输结果}{Transmission results for two image-classification data distributions over time-varying satellite--ground links}
...
本章针对轻量星上编码器单轮传输能力受限且模型规模难以继续扩展的问题，提出 CIS-R 机制，以接收端任务反馈引导的选择性增量通信替代星上模型扩展。在线传输阶段，星上发送端无需部署或访问下游任务模型，任务模型推理与反馈计算均在地面端完成。实验结果显示，随着基础轮任务准确率提高，后续增量传输的边际增益逐步下降；消融实验中，移除任务驱动反馈、反馈与轮次条件化特征变换或自适应语义累积均导致最终准确率下降。在所测试配置中，$K=16$ 和三轮增量重传获得较高的准确率--CBR 增益比。

在高压缩率图像传输条件下，三轮增量传输以约 $24.5\%$ 的累计 CBR 增幅换取 $12.24$ 个百分点的 Top-1 分类准确率增益，给出了增量通信开销与任务性能之间的定量交换关系。在 LEO 卫星过顶北京密云、喀什和三亚地面站的星地图像传输仿真中，当采用相同的前向传输与反馈开销时，任务驱动反馈较随机反馈将两种图像分类数据分布上的平均分类准确率提高 $4.5$--$5.4$ 个百分点，说明基于当前任务判决敏感度的分块优先级在所测试的时变星地链路配置下均取得更高的任务准确率。该实验还表明，在保持星上发送参数不变的条件下，地面接收端适配能够支持新的图像分类数据分布；其结论范围不扩展至分类、检测和分割等不同任务类型之间的迁移。在当前实现的统计口径下，CIS-R 通信模型参数量约为生成式 DiffJSCC 方案的 $1/2400$。
