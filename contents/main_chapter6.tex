...
为解决上述问题，本章提出协同增量语义重传（Coordinated Incremental Semantic Retransmission，CIS-R）机制。其核心思路是在不扩展星上通信模型规模的条件下，由地面接收端依据当前任务判决生成分块选择反馈，控制后续轮次传输的分块位置，并通过跨轮接收状态累积提高任务性能。CIS-R 的星上发送端在在线传输阶段无需部署或访问下游任务模型、任务输出和任务标签，任务模型推理、分块优先级计算与反馈生成均在地面接收端完成。ImageNet-100 仿真与时变星地链路实验表明，在所测试的静态 AWGN 信道和时变 SNR 轨迹下，任务驱动的分块选择相较随机选择取得了更高的任务准确率。本章相关成果已撰写成会议论文 ``CIS-R: Receiver-Driven Incremental Semantic Retransmission for Resource-Constrained LEO Image Delivery''，投稿于 IEEE International Conference on Computer Communications（INFOCOM）2027。
...
\end{aligned}
\end{equation}
因此，基础轮和增量轮均满足单位平均发送功率约束。接收端首先按照反馈索引将本轮接收的 $K$ 个有效分块信道表征回填至 $N\times\epsilon$ 的零填充信道表征，再输入与基础轮共享参数的语义解码器。解码得到完整的 $N\times D$ 分块表征后，硬掩码仅保留被请求位置的解码结果，并与历史语义状态累积。未被请求的分块不占用本轮前向链路资源，其历史状态在本轮保持不变。
...
算法~\ref{algo:ch6_cis} 给出了 CIS-R 的多轮收发顺序。基础轮中，卫星发送归一化基础负载，地面端解码得到初始分块级接收表征。第 $r$ 个增量轮开始时，地面端计算分块优先级并反馈 Top-$K$ 掩码；卫星端生成、打包和归一化对应的语义增量；地面端依次完成接收信道表征回填、共享解码器解码、硬掩码提取和门控累积。完成 $R$ 轮后，地面端输出任务结果。
...
表~\ref{tab:ch6_complexity_latency} 中的 GPU 任务处理时延表示在 RTX A6000 上，从输入图像进入通信模型开始，到输出 R3 分类结果为止的完整 GPU 计算时间。CIS-R 的测量路径包括基础轮与三个增量轮的编码和解码、四次任务模型前向推理、三次基于梯度的分块归因计算、分块选择及接收状态融合，不包括星地传播、前向符号传输、反馈链路传输、模型加载、主机与 GPU 数据搬移和文件读写。因此，该指标属于任务处理计算时延，不等同于端到端星地业务时延。测试批量大小为 $1$，数据精度为 FP32。参数量统计通信模型，峰值显存覆盖当前实现的完整任务处理链路。DiffJSCC 使用 $512\times512$ 输入和 $50$ 个去噪步骤，其余方法的输入尺寸均为 $224\times224$；CIS-R 采用 $\epsilon=64$，并执行 R0 至 R3 的完整收发流程。由于输入尺寸、迭代次数和多轮交互过程不同，表中数据用于比较当前实现的模型规模与 GPU 任务处理开销，不构成同构网络结构下的严格复杂度比较。

\begin{table}[!htp]
    \centering
    \bicaption{不同方法当前实现的模型规模与 GPU 任务处理开销}{Model size and GPU task-processing cost of current implementations}
    ...
    \begin{tabular}{@{}lcccc@{}}
        \toprule
        方法 & 输入尺寸 & 参数量（M） & GPU 任务处理时延（ms） & 峰值显存（MiB） \\
        \midrule
...
在上述统计口径下，CIS-R 的通信模型参数量为 $2.274$~M，峰值显存约为 $150$~MiB，均为表中最低。完整 R0--R3 GPU 任务处理时间为 $45.340$~ms。SwinJSCC 和 Codebook 只执行单次前向处理，GPU 任务处理时延低于 CIS-R；DiffJSCC 的生成式恢复包含多次迭代，GPU 任务处理时延达到 $4382.750$~ms。CIS-R 将任务模型推理和反馈计算部署在地面端，星上仅执行基础编码与语义增量生成。表中时延和显存由完整通信模型在地面 GPU 上测得，不直接代表星上硬件的推理开销。

\noindent(2)~\textbf{时变 LEO 星地传输性能}

时变星地链路仿真采用卫星轨道几何和链路预算生成下行链路状态，地面站包括北京密云、喀什和三亚。前向链路符号速率设为 $4\times10^8$ 复信道符号/s，反馈速率为 $1$~Mbit/s，单轮反馈周转时间为 $4$~ms。

业务负载由 $500$ 幅 ImageNet-100 图像和 $500$ 幅 CIFAR-100~\cite{krizhevsky-CIFAR-TR09} 图像交替组成。两类图像均经过同一个在 ImageNet-100 上训练并冻结的星上发送端，星上基础编码器、轮次条件化表征生成模块及其余发送参数在 CIFAR-100 任务中均不更新。ImageNet-100 接收端沿用原始训练参数；对于 CIFAR-100，首先在冻结共享发送端的条件下训练任务专用的语义解码器和反分块映射，使其能够恢复共享发送端产生的基础表征，随后冻结上述模块，仅训练接收端融合门以适配多轮补充表征。两类任务分别使用对应的地面任务模型完成推理与反馈计算。该设置用于考察固定星上发送参数通过接收端适配支持异构下游任务的能力，而不表示两个任务共享完全相同的端到端收发参数。任务驱动反馈和随机反馈使用相同的前向、反向负载，并对每幅图像、每个轮次采用相同的 AWGN 噪声样本。两类任务的汇总准确率列于表~\ref{tab:ch6_trace_case}。

时变链路实验采用在固定 SNR $=5$~dB 的 AWGN 信道下训练得到的模型，并直接在由卫星轨道几何和链路预算生成的时变 SNR 轨迹上测试，测试期间不重新训练模型，也不执行在线信道自适应。每个增量轮开始时，仿真器读取当前 SNR 和星地斜距，根据当前 SNR 生成 AWGN，并由斜距计算单向传播时延。因此，该实验反映的是固定信道条件下训练的 CIS-R 模型对所测试时变 SNR 轨迹的直接泛化性能。
...
在前向和反馈负载完全相同的条件下，任务驱动反馈相较随机反馈在北京密云、喀什和三亚链路上分别提高 $5.4$、$5.4$ 和 $4.5$ 个百分点。在可见窗口内 SNR 和传播距离持续变化的情况下，基于当前任务判决敏感度的分块优先级仍取得更高的任务准确率。表中业务时延由解析链路仿真器计算，与表~\ref{tab:ch6_complexity_latency} 中实测的 GPU 任务处理时延属于不同统计口径。

\section{本章小结}

本章针对轻量星上编码器单轮传输能力受限且模型规模难以继续扩展的问题，提出 CIS-R 机制，以接收端任务反馈引导的选择性增量通信替代星上模型扩展。在线传输阶段，星上发送端无需部署或访问下游任务模型，任务模型推理与反馈计算均在地面端完成。实验结果显示，随着基础轮任务准确率提高，后续增量传输的边际增益逐步下降；消融实验中，移除任务驱动反馈、反馈条件化调制或自适应语义累积均导致最终准确率下降。在所测试配置中，$K=16$ 和三轮增量重传获得较高的准确率--CBR 增益比。
